  ...
  <body>
    <div class='main'>
      ...
      <form method="post">
        ...
        <input type="password" pattern="[a-zA-Z0-9]+" name="password" placeholder="رمز عبور" required />
        <input type="email" name="email" required placeholder="ایمیل" />
        <button type="submit" class="button" name="login" name="register">ثبت نام</button>
        <p class="text-center mt-3" style="font-size:11.5pt;color:white;">اگر حسابی دارید <span class="text-primary" style="cursor:pointer;" href="javascript:void(0)" onclick="goToURL(); return false;">ورود</span> کنید</p>
      </form>
    </div>
    <?php
      ...
      if (isset($_POST['username']) && isset($_POST['password']) && isset($_POST['email'])) {
          ...
          if ($query->rowCount() > 0) {
            ...
            echo '<div class="jumbotron error p-4 align-items-center">';
            echo '<p class="text-light text-center">این ایمیل قبلا ثبت شده است</p>';
            echo '<button class="w-100 close button">باشه</button>';
            echo '</div>';
          }
          ...
          if ($query->rowCount() == 0) {
              ...
              if (!$result) {
                echo '<script>$(".main").css("display" , "none");</script>';
                echo '<div class="jumbotron error p-4 align-items-center">';
                echo '<p class="text-light text-center">نام کاربری یا رمز عبور قبلا ثبت شده است</p>';
                echo '<button class="w-100 close button">باشه</button>';
                echo '</div>';
              } else {
                echo '<script>$(".main").css("display" , "none");</script>';
                echo '<div class="jumbotron p-4 success align-items-center">';
                echo '<p class="text-light text-center">ثبت نام با موفقیت انجام شد.حال باید ورود کنید</p>';
                echo '<button class="w-100 button" href="javascript:void(0)" onclick="goToURL(); return false;">ورود</button>';
                echo '</div>';
              }
          }
      }
